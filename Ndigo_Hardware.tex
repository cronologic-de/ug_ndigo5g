...
\textbf{Caution:} In some cases, modern PC mainboards have PCIe slots with an additional metal shielding. Be careful not to cause an electric short when installing the board as some board components (see Figure~\ref{fig:pcie-plug}) are located close to the PCIe connector. 

\begin{figure}[htb]
    \centering
    \includegraphics[width=0.6\textwidth]{figures/pcie-plug.jpg}
    \caption{Close-up of the Ndigo5G-10 PCIe-Plug. Make sure that the highlighted elements do not get in contact with any metal parts of your
    mainboard's PCIe slot.}
    \label{fig:pcie-plug}
\end{figure}
