...
\section{Constants}
    ...
    \label{sec:initialization}
        ...
        \subsection{Structure ndigo\tu init\tu parameters}
            ...
            When true, the corresponding pins of connector (see Table~\ref{table:c2-pinout})
            must be supplied by a differential 10~MHz clock signal with a common mode
            of about 1200~mV and at least 150~mV voltage difference.\par
            ...
            Defaults to false.\par
            When true, the corresponding pins of connector (see Table~\ref{table:c2-pinout})
            output a differential LVDS 10~MHz clock signal with a common mode of
            1000~mV and a voltage difference of at least 480~mV.

            \cronvar{ndigo\tu bool\tu t}{is\tu slave}\\
            Data acquisition of this board is controlled by the master board.\par
            Defaults to false.\par

            \cronvar{ndigo\_bool\_t}{mutliboard\_sync}\\
            Set whether the board is used in  a multiple boards environment.\par
            Defaults to false.\par

            \cronvar{int}{direct\_sync}\\
...
%			Unused portion of the host buffer in bytes.\par
...
first release.\par
    ...
    \section{Configuration}
        ...
        \subsection{Structure ndigo\tu configuration}
            ...
            \crondef{NDIGO\tu ADC\tu MODE\tu C12 30} // not available on all boards\\
            ...
            Defaults to 3\,GHz (1\,GHz for the -250M variant).
            \par
        ...
        \cronvar{ndigo\tu bool\tu t}{force\tu led}\\
        Activates LED on input, even if trigger block is not enabled.\par
...
trigger condition is fulfilled.\par
        ...
        \subsection{Run Time Control}
            ...
            \cronvar{int}{ndigo\tu single\tu shot(}\cronvar{ndigo\tu device}{*device}, \cronvar{int}{channel\tu mask)}\\
            Enables each of the specified channels for one trigger, respectively.

            \cronvar{int}{ndigo\tu manual\tu trigger(}\cronvar{ndigo\tu device}{*device}, \cronvar{int}{channel\tu mask)}\\
            Enables manual triggering of the specified channels, respectively.
    ...
    \section{Readout\label{cp:readout}}
        ...
        This feature allows to either free up partial host-buffer space early if there will be no call to \textsf{ndigo\tu read} anytime soon. It also allows keeping data over multiple calls to \textsf{ndigo\tu read} to avoid unnecessary copying of data.\par
