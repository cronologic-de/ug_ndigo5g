...
%
The inputs of the Ndigo5G are located on the PCI bracket. Figure~\ref{fig:schematics} on page~\pageref{fig:schematics} shows the location of the 4 analog inputs A to D and the two digital inputs G (GATE) and T (Trigger). Furthermore, two board interconnection connectors can be found at the top edge of the Ndigo5G, as displayed in  Figure \ref{fig:schematics} on page \pageref{fig:schematics}. Connector C1 is used for a board-to-board connection (e.g., to link a HPTDC8-PCI and a Ndigo5G via a Ndigo Extension board, see chapter \ref{cp:extcard}). Connector C2 is used as a bus interface between multiple Ndigo boards distributing clock, trigger and sync signals. It can be used to supply the Ndigo5G with an external 10 MHz clock or to output such a signal (depending on the initialization parameters, see Section~\ref{sec:initialization}). Refer to Table~\ref{table:c2-pinout} for the pinout. Proper termination must be placed at both ends of the bus interconnection ribbon cable.
%
\begin{figure*}[hb]
    \centering
...
\begin{table}
        ...
        \centering
            \begin{tabular}{|c|c|}
                ...
                1, 2 & GND\\
                \hline
                3, 4 & ext. CLK in (or out) N, ext. CLK in (or out) P\\
                \hline
                5, 6 & GND\\
                \hline
                7, 8 & reserved/NC\\
                ...
                \hline
            \end{tabular}
            \caption{Pinout of connector C2.}
            \label{table:c2-pinout}
    \end{small}
\end{table}
%
%
...
\subsection{Gating Blocks}
        ...
        To decrease the amount of data transmitted to the PC, Ndigo5G includes 4 independent gate and delay units. A gate and delay unit creates a gate window starting at a specified time after a trigger, closing the window at gate stop. Both timing values — gate start and gate stop — must be set as multiples of 3.2~ns.\par
